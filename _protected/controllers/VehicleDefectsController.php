...
class VehicleDefectsController extends Controller
{
     ...
     * @return mixed
     */
    public function actionIndex($remove = '')
    {
...
//        var_dump($avtomodel); die();
    ...
    public function actionDefectxorazm(){

        $model = new Vehicles();
        if ($model->load(Yii::$app->request->post())) {
//            print_r('sasas'); die();
            $con = new Mssql();
            $mssql = $con->selectSash($model->pono);
//            var_dump($mssql); die();
            return $this->redirect(['defectxorazm', 'mssql' => $mssql, ]);
        }
        $shops = Shops::find()->asArray()->all();
        $level1 = Level1::find()->asArray()->all();
        $defect = DefectList::find()->asArray()->all();
        ...
        return $this->render('defectxorazm', [
            'model' => $model,
            'shops' => $shops,
            'level1' => $level1,
            'defect' => $defect,
